...
**Load-bearing caveat (CORRECTED 2026-08-19).** The 33.6% baseline used
throughout this report was measured in May against a hosted endpoint and reused
as the reference all summer. When remeasured in the same epoch as the treatments,
the no-memory 8B baseline is **39.3–41.4%** (three matched-epoch replicates:
55/140, 55/140, 58/140). The ~6pp gap is hosted-endpoint non-stationarity, not
a curation effect. **Every "significant peak" in the tables below was computed
against 33.6% and is inflated by this drift.** Paired lifts against a
contemporaneous baseline are 3–8pp smaller, and none survives multiplicity
correction at any threshold. The tables are retained as measured but must be read
against a ~39–41% reference, not 33.6%. See DIVERGENCES item 16, JOURNAL
RETRACTION 1/2.
