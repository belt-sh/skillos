...
OUT=output/alfworld-8xh100-algo1-fft-seed3
EVAL=output/eval-fft-seed3
...
storm_count () {
  local tot=0 n f
  for f in /tmp/seed3_ckpt*.log; do
    [ -f "$f" ] || continue
...
run_wave () {  # up to 4 ckpts, GPU-pinned; returns 1 if storm-aborted
  ...
  for ck in "$@"; do
    CUDA_VISIBLE_DEVICES=$gpu .venv/bin/python -u -m scripts.eval_streaming_curation \
      ...
      --out "$EVAL/ckpt$ck.jsonl" > "/tmp/seed3_ckpt$ck.log" 2>&1 &
    pids+=($!)
    gpu=$((gpu+1))
  done
  while true; do
...
while true; do
  ...
  gate
  rm -f /tmp/seed3_ckpt*.log   # reset storm counter for the new attempt
