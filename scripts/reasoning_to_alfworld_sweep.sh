#!/usr/bin/env bash
# Cross-domain transfer: reasoning-trained curator → ALFWorld eval.
# 8-arm waves, storm-resilient. Paper Table 3 claims +13.3pp.
set -u
cd "$(dirname "$0")/.."

TRAIN_OUT=output/reasoning-8xh100-algo1-fft
EVAL=output/eval-reasoning-to-alfworld
...
gate () {
  ...
  while true; do
    R=$(probe_round)
    echo "[$(date -u)] gate: $R (streak=$streak)" >> logs/reasoning_to_alfworld_sweep.log
    if [ "$R" = "PASS" ]; then
      streak=$((streak+1))
      [ "$streak" -ge 2 ] && { echo "[$(date -u)] gate OPEN" >> logs/reasoning_to_alfworld_sweep.log; return 0; }
      sleep 300
    else
      streak=0
      sleep 300
...
run_wave () {
  ...
  for ck in "$@"; do
    CUDA_VISIBLE_DEVICES=$gpu .venv/bin/python -u -m scripts.eval_streaming_curation \
      --mode closed_loop --curator-checkpoint "$TRAIN_OUT/checkpoint-$ck" \
      --num-games 140 --batch-size 20 --split valid_seen \
      --curator-device cuda --curator-temperature 0 --curator-max-new-tokens 1536 \
      --out "$EVAL/ckpt$ck.jsonl" > "logs/r2a_ckpt$ck.log" 2>&1 &
    pids+=($!)
    gpu=$((gpu+1))
  done
  echo "[$(date -u)] wave PIDs: ${pids[*]}" >> logs/reasoning_to_alfworld_sweep.log
  ...
  while true; do
    ...
    if [ "$(storm_count)" -gt "$STORM_THRESHOLD" ]; then
      echo "[$(date -u)] STORM ($(storm_count) errors) — killing wave" >> logs/reasoning_to_alfworld_sweep.log
...
echo "[$(date -u)] reasoning→ALFWorld sweep started" >> logs/reasoning_to_alfworld_sweep.log

while true; do
  REMAINING=()
  for ck in $ALL_CKPTS; do arm_ok "$ck" || REMAINING+=("$ck"); done
  if [ "${#REMAINING[@]}" -eq 0 ]; then break; fi
  echo "[$(date -u)] remaining arms: ${REMAINING[*]}" >> logs/reasoning_to_alfworld_sweep.log

  gate
  rm -f logs/r2a_ckpt*.log
  ...
  while [ "$#" -gt 0 ]; do
    ...
    echo "[$(date -u)] WAVE: ${wave[*]}" >> logs/reasoning_to_alfworld_sweep.log
    if ! run_wave "${wave[@]}"; then stormed=1; break; fi
    echo "[$(date -u)] wave done: ${wave[*]}" >> logs/reasoning_to_alfworld_sweep.log
  done
  [ "$stormed" -eq 1 ] && { echo "[$(date -u)] back to gate" >> logs/reasoning_to_alfworld_sweep.log; sleep 300; }
done

echo "[$(date -u)] ALL ARMS COMPLETE" >> logs/reasoning_to_alfworld_sweep.log

CMP_ARGS=(--arm "no_memory=$BASE")
for ck in $ALL_CKPTS; do CMP_ARGS+=(--arm "ckpt$ck=$EVAL/ckpt$ck.jsonl"); done
.venv/bin/python -m scripts.compare_eval_arms "${CMP_ARGS[@]}" | tee "$EVAL/comparison.txt"
echo "[$(date -u)] SWEEP COMPLETE" >> logs/reasoning_to_alfworld_sweep.log
