#!/usr/bin/env bash
# SkillOS Algorithm 1 FFT — within-group easy->hard curriculum (paper Table 5).
...
#
# Usage:
#   ./run_algo1_fft_curriculum.sh                 # full run (max_steps from config)
#   ./run_algo1_fft_curriculum.sh <checkpoint>    # resume
...
export WANDB_RUN_ID="${WANDB_RUN_ID:-algo1fftcurriculum}"
...
LOG="logs/alfworld_8xh100_algo1_fft_curriculum_${ts}.log"
mkdir -p logs
...
accelerate launch \
  ...
  -m scripts.train_algo1 --config configs/alfworld_8xh100_algo1_fft_curriculum.yaml \
  > "$LOG" 2>&1
