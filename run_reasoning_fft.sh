...
export WANDB_RUN_ID="${WANDB_RUN_ID:-reasoningfft}"
...
LOG="logs/reasoning_8xh100_algo1_fft_${ts}.log"
mkdir -p logs
echo "Launching SkillOS Reasoning Algorithm 1 FFT (ZeRO-3 + beta=0.001) → $LOG"
echo "  CHECKPOINT=${CHECKPOINT:-<fresh>}  PHASE_BUDGET_S=$SKILLOS_PHASE_BUDGET_S"

accelerate launch \
  --config_file configs/accelerate_zero3.yaml \
  -m scripts.train_reasoning --config configs/reasoning_8xh100_algo1_fft.yaml \
  > "$LOG" 2>&1
